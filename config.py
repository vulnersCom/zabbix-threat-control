...
acknowledge_users = config.get(
    "OPTIONAL", "TrustedZabbixUsers", fallback="Admin"
).split(",")
ssh_user = config.get("OPTIONAL", "SSHUser", fallback="root")

log_file = config.get(
    "OPTIONAL", "logfile", fallback="/var/log/zabbix-threat-control.log"
)
debug_level = config["OPTIONAL"].getint("DebugLevel", 1)
work_dir = config.get(
    "OPTIONAL", "WorkDir", fallback="/opt/monitoring/zabbix-threat-control"
).rstrip("/")

application_name = config.get(
    "OPTIONAL", "HostsApplicationName", fallback="Vulnerabilities"
)
...
bulletins_name = config.get(
    "OPTIONAL", "BulletinsVisibleName", fallback="Vulners - Bulletins"
)

packages_host = config.get("OPTIONAL", "PackagesHost", fallback="vulners.packages")
packages_name = config.get(
    "OPTIONAL", "PackagesVisibleName", fallback="Vulners - Packages"
)

statistics_host = config.get(
    "OPTIONAL", "StatisticsHost", fallback="vulners.statistics"
)
statistics_name = config.get(
    "OPTIONAL", "StatisticsVisibleName", fallback="Vulners - Statistics"
)

stats_macros_name = config.get(
    "OPTIONAL", "StatisticsMacrosName", fallback="{$WORK_SCRIPT_CMD}"
)
stats_macros_value = config.get(
    "OPTIONAL",
    "StatisticsMacrosValue",
    fallback="/opt/monitoring/zabbix-threat-control/scan.py",
)

template_host = config.get(
    "OPTIONAL", "TemplateHost", fallback="tmpl.vulners.os-report"
)
template_name = config.get(
    "OPTIONAL", "TemplateVisibleName", fallback="Template Vulners OS-Report"
)

template_macros_name = config.get(
    "OPTIONAL", "TemplateMacrosName", fallback="{$REPORT_SCRIPT_PATH}"
)
template_macros_value = config.get(
    "OPTIONAL", "TemplateMacrosValue", fallback="/opt/monitoring/os-report/report.py"
)

template_application_name = config.get(
    "OPTIONAL", "TemplateApplicationName", fallback="Vulners OS Report"
)
